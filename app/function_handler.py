...
def validate_tts_output(text: str) -> dict:
    """
    Final validation before TTS. Returns {"safe": bool, "text": str, "violations": list}
    Rejects responses with forbidden patterns.
    """
    violations = []
    
    # Check for forbidden patterns
    forbidden_patterns = [
        'initiate_', 'collect_', 'confirm_and_', 'n8n', 'webhook', 'api',
        'function', 'async', 'await', 'return', 'error code', 'database'
    ]
    ...
    if '{' in text or '[' in text or '<' in text:
        violations.append('template_variables')
    
    if violations:
        logger.error(f"TTS VIOLATION DETECTED: {violations} in text: {text}")
        return {
            "safe": False,
            "text": "I'm here to help. What would you like to do next?",
            "violations": violations
        }
    
    return {"safe": True, "text": text, "violations": []}
...
def format_prompt(template: str, params: Dict[str, Any]) -> str:
    ...
    # FINAL VALIDATION - Runtime TTS check
    validation = validate_tts_output(sanitized)
    if not validation["safe"]:
        logger.error(f"TTS output rejected: {validation['violations']}")
        return validation["text"]  # Safe fallback
...
async def handle_function_call(
    ...
    """
    # CRITICAL: Log all function calls for monitoring
    logger.info(f"🔧 FUNCTION CALL: {function_name} | Session: {session_id} | Args: {arguments}")
    
    # Get conversation context for adaptive responses
    context = get_context(session_id)
    
    # VALIDATION: Check if function call is appropriate (early detection)
    if function_name.startswith('initiate_') and len(context.history) < 2:
        # User just connected, probably shouldn't be calling initiate functions yet
        logger.warning(f"⚠️  Suspicious early function call: {function_name} on turn {len(context.history)}")
    ...
    if function_name == "initiate_booking":
        ...
        logger.info(f"📅 Booking initiated with intent: {user_intent}")
        ...
        raw_phone = arguments.get("client_phone", "")
        context = get_context(session_id)
        
        # Validate phone number
        validation = validate_phone_number(raw_phone, default_country_code="234")
        
        if validation["valid"]:
            # Store formatted E.164 number
            session["params"]["client_phone"] = validation["formatted"]
            session["params"]["phone_spoken"] = validation["spoken"]
            
            # Read back for confirmation
            context.reset_retry()  # Success, reset retry counter
            return {"speak": sanitize_for_tts(f"I have {validation['spoken']}. Is that correct?")}
        else:
            # Invalid format - ask again with guidance
            context.increment_retry()
            
            if context.retry_count >= 3:
                # Too many retries, offer human transfer
                return {"speak": sanitize_for_tts("I'm having trouble getting your phone number. Let me connect you with our team who can help.")}
            
            error_msg = validation.get('error', 'I didn\'t catch that phone number.')
            return {"speak": sanitize_for_tts(f"{error_msg} Please try again.")}
    ...
    # =========================================================================
        ...
        response_text = arguments.get("response_text", "I'm here to help.")
        # Full validation chain: Sanitize -> Validate -> Fallback if needed
        sanitized = sanitize_for_tts(response_text)
        validation = validate_tts_output(sanitized)
        
        if not validation["safe"]:
            logger.error(f"TTS output rejected in continue_conversation: {validation['violations']}")
            return {"speak": validation["text"]}  # Safe fallback
            
        return {"speak": sanitized}
